...
export class AttributeKey {
    static create( attrKey: AttributeKey | string ): AttributeKey {
        if( typeof attrKey === typeof AttributeKey ) {
            return attrKey as AttributeKey;
        } else {
...
class AttributeSchema {
    ...
    protected children: { [ key: string ]: AttributeSchema } = {};
    protected keys: string[] = [];

    constructor( attrKey: AttributeKey | string ) {
        this.key = AttributeKey.create( attrKey );
    }
    
    // スキーマを編集してしまうと、値が入ってしまうので、コピーを作る必要がある。
    protected cloneSchema(): AttributeSchema {
        let schema: AttributeSchema;
        for( let key in this.children ) {
            if( typeof this.values[ key ] === typeof Attribute ) {
                schema.children[ key ] = this.children[ key ].cloneSchema();
            } else {
                newAttr.values[ key ] = this.values[ key ];
            }
        }
...
class ExtendableAttributeSchema {
    protected key;
    // キーを追加する (値は未設定、構造だけの変更)
    addKey( attrKey: AttributeKey ) {
        this.values[ attrKey.key ] = undefined;
